...
public:
  ...
  virtual std::string get_type_name() { return type_name; }
...
{
public:
  leaf_geneartort(exprst le, std::string n = "")
  {
    leaf_exprs = le;
    name = n;
    allowed_size = 0;
...
public:
  ...
  non_leaf_geneartort(generatorst sg, std::string n = "")
  {
...
std::list<exprst> cartesian_product_of_generators(generatorst &generators, size_t pos = 0)
{
...
{
  //  generate all splits of n into k components. Order of the split components is considered relevant.
  assert(n >= k);
  ...
  non_leaf_geneartort* clone() override { return new non_leaf_geneartort(sub_generators); }

  virtual bool commutative_check(exprst es) { return true; }

protected:
  void set_sub_generator_sizes(sizest sizes)
  {
    assert(sizes.size() == arity);
  ...
  virtual exprt instantiate(exprst exprs) { return true_exprt(); };
  size_t arity;
  size_t allowed_size;
...
public:
  ...
  binary_functional_generatort(irep_idt op, generator_baset & op1, generator_baset & op2, std::string n = "")
  {
    name = n;
    sub_generators = generatorst();
  ...
  {
    if ((op_id == ID_equal || op_id == ID_le || op_id == ID_lt || op_id == ID_and) && from_expr(exprs.front()) == from_expr(exprs.back()))
      return false;
    ...
    //if (hash_fun(from_expr(exprs.front())) == hash_fun(from_expr(exprs.back())))
...
{
public:
  alternatives_generatort(generatorst sg)
  {
    sub_generators = generatorst();
    for (auto g: sg)
    {
  ...
  alternatives_generatort* clone() override { return new alternatives_generatort(sub_generators); }
...
public:
  ...
  recursive_generator_placeholdert(generator_factory_baset *fac, std::string id, std::string n = "")
  ...
  recursive_generator_placeholdert* clone() override { return new recursive_generator_placeholdert(factory, identifier); }

  std::string identifier;

protected:
  bool eq(recursive_generator_placeholdert other){ return other.identifier == identifier;}
  bool ne(recursive_generator_placeholdert other){ return other.identifier != identifier;}
...
public: 
  ...
  {
    assert(generator_map.count(identifier) == 0);
    recursive_generator_placeholdert result = recursive_generator_placeholdert(this, identifier);
    generator_map[identifier] = result;
    return result;
  }

  recursive_generator_placeholdert make_generator(std::string generator_name, generatorst arg_geneartors)
  {
  ...
  virtual generator_baset* instantiate_placeholder(recursive_generator_placeholdert) = 0;
...
{
public:
  recursive_geneartor_factoryt()
  : generator_factory_baset()
  {
  }

  alternatives_generatort* instantiate_placeholder(recursive_generator_placeholdert placeholder) override
  {
